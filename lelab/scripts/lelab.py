...
def _find_lelab_pids() -> dict[int, str]:
    ...
    never touch unrelated dev servers:
      1. cmdline runs `uvicorn ... lelab.alex_server`  (the reload supervisor / prod)
    ...
    for proc in psutil.process_iter(["pid", "cmdline"]):
        ...
        if "lelab.alex_server" in cmdline:
            targets[pid] = "uvicorn (lelab.alex_server)"
            continue
