...
def test_inspect_dataset_reports_alex_schema_and_standard_training_readiness(tmp_path: Path) -> None:
    import json

    from lelab.datasets import inspect_dataset

    dataset = tmp_path / "alex"
    (dataset / "meta").mkdir(parents=True)
    (dataset / "meta" / "tasks.parquet").write_bytes(b"")
    ...
    result = inspect_dataset(path=str(dataset))
    assert result["layout"] == "v3"
    assert result["conversion"]["gr00t_required"] is False
    assert result["conversion"]["lerobot_ready"] is True
    assert result["cameras"] == ["observation.images.cam_zed_left"]
    assert {item["name"]: item["dimension"] for item in result["feature_schema"]}["action"] == 46
